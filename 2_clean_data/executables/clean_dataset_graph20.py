    ...
    for file_name in [file for file in os.listdir(source_path)
        ...
        with open(source_path + file_name) as json_data:
            ...
            for x in range(len(data["review_content"])):
                reviews.append("graph20_" + str(paper_id) + "_" + str(x + 1))
                reviews.append(clean_data(data["review_content"][x]["review"]))
                graph20_rating = int((data["review_content"][x]["rating"]).split(":")[0])  # noqa
                if graph20_rating == 2 or graph20_rating == 3:
                    graph20_rating = 1
                elif graph20_rating == 4 or graph20_rating == 5:
                    graph20_rating = 2
                elif graph20_rating == 6 or graph20_rating == 7:
                    graph20_rating = 3
                elif graph20_rating == 8 or graph20_rating == 9:
                    graph20_rating = 4
                else:
                    print("Error: The rating of review " + str(x + 1)
                          + " of paper " + str(paper_id)
                          + " does not have a rating from 2-9!"
                            "\nThe rating has been set to 1.")
                    graph20_rating = 1
                reviews.append(graph20_rating)
